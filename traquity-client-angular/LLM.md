...
one the extraction request carries — and `classify-cells.ts` marks an amount only where exactly that code stands beside it. There is no
table of currency codes here and none is to be added: every other word is text, so a figure stated in another currency reaches the model
looking like a rate or a reference number, and telling those apart is the prompt's job.
...
fraction, `03/04` is two days of the calendar); `parse<X>` takes the first of them, which is what a token carries. A notation added here
...
`@Input`/`@Output` pair, because `input()` and `output()` need an injection context and this suite runs in a node environment with no
TestBed.
