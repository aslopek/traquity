...
- **Startup screens and the app shell**: `AppComponent` is just a `<router-outlet>` plus the trigger that starts the backend immediately
  ...
  `src/app/startup/` holds the startup flow itself. `StartupStore` (a root-provided `@ngrx/signals` Signal Store, not a global-store slice —
  it is read by an app initializer, a route guard and both startup screens) holds the startup mode, phase, database path, the database's
  `authState` (what gates the unlock screen's `OK` button: only a stored `scrypt` record demands a local match) and the `startFailed` flag
  (set on a failed `backend:start`, cleared on the next attempt), and drives the `backend:start` call. `selectDatabase` is what keeps
  ...
  bridge is available, that writes the one-shot `configureOnNextStart` flag and relaunches the app through `StartupBridgeService`.
...
the one-line-per-row text a language model reads. `tokens-of-document.ts` states every value that document states — an id, a kind, the
text as printed, the normalized value, the label naming it and the currency it is denoted in — and `isin-of-document.ts` the security it
names, where it names exactly one. `label-of-token.ts` is the one rule for what names a value.
...
enumerates every reading, the likelier one first, because a notation is genuinely ambiguous often enough that committing is a decision
someone else has to make (`1.005` is a thousand and five or a fraction, `03/04` is two days of the calendar); `parse<X>` takes the first of
them, which is what a token carries. The main process is handed the tokens and reads no notation itself, so a notation added here reaches
the grammar with no second edit anywhere, and a notation missing here is missing everywhere.

Three things about it are load-bearing and not preferences. **The tolerances in the geometry stages are tuned constants**, and no unit
test can tell a good split from a bad one, so one changed by hand is a regression the suite cannot catch. The parse is
