...
import {extractPdf, PdfExtractionFailure, PdfExtractionResult, PdfReading, readingOfDocument} from "../../../../../common";
 ...
 * unlike a run that failed, so this is the only wording available for it - and it names no document, because the
 * document parsed a moment earlier and nothing about it is in question.
 */
...
function extracted(signalStore: WritableSignalStore<TransactionImportState, TransactionImportComputed>,
  ...
  return from(extractPdf(bytes, args.currency)).pipe(
    switchMap((result: PdfExtractionResult): Observable<void> => {
      ...
      const reading: PdfReading = readingOfDocument(result.document);
