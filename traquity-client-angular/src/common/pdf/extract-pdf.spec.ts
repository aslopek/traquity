...
describe("extractPdf", (): void => {
  ...
  it("produces one page carrying the row the items stood on", async (): Promise<void> => {
    const result: PdfExtractionResult = await extractPdf(bytes, currency, bounds);
  ...
  it("passes the bytes to pdf.js", async (): Promise<void> => {
    await extractPdf(bytes, currency, bounds);
  ...
  it("reads the amounts in the currency it was given", async (): Promise<void> => {
    itemsByPage = [[
      itemFactory(),
    ...
    expect(result.status === "extracted" && result.document.pages[0].rows[0].cells[1].tokens)
      .toEqual([{text: "1.005,00", label: null, magnitude: 1005, digits: 2, currency, last: false}]);
  });
