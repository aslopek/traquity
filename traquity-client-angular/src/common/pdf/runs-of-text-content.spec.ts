...
describe("runsOfTextContent", (): void => {
  ...
  it("flips the y axis against the page height, so a smaller y is higher", (): void => {
    // the item as it stood before the act, so the expectation is no view of what the act itself produced
    const item: PdfTextItem = {...items[0], transform: [...items[0].transform]};

    expect(runsOfTextContent(items, PAGE_HEIGHT)).toEqual([{
      text: item.str,
      x: item.transform[4],
      y: PAGE_HEIGHT - item.transform[5],
      width: item.width,
      height: item.height,
