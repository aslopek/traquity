...
describe("parseNumber", (): void => {
  ...
  describe("notation", (): void => {
    ...
    it("reads an English amount, the comma grouping and the dot dividing", (): void => {
      expect(parseNumber("1,005.00")).toEqual({magnitude: 1005, sign: null, digits: 2});
    });

    it("keeps every printed decimal", (): void => {
    ...
    it("reads the typographic apostrophe as a group and never as a decimal point", (): void => {
      expect(parseNumber("1’005.00")).toEqual({magnitude: 1005, sign: null, digits: 2});
    });
  ...
  describe("booking direction", (): void => {
    ...
    it("reports a trailing plus apart from the magnitude", (): void => {
