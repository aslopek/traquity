...
function rowOf(runs: PdfRun[]): PdfRunRow {
  return {
    y: 429,
    height: Math.max(...runs.map((run: PdfRun): number => run.height)),
    runs
  };
}
...
describe("cellsOfRow", (): void => {
  ...
  it("joins runs a wider gap apart into one cell, with a space between them", (): void => {
    ...
    runs = [
      first,
      after(first, CELL_GAP, {text: "Betrag"})
    ];
  ...
  it("opens a new cell where the gap exceeds the threshold", (): void => {
    ...
    runs = [
      first,
      after(first, SPLIT_GAP, {text: "1.005,00"})
    ];
    expect(cellsOfRow(rowOf(runs)).map((cell: PdfCellBox): string => cell.text)).toEqual(["Kurswert", "1.005,00"]);
  });

  it("scales the threshold to the taller run, so the same gap joins into one cell", (): void => {
    const first: PdfRun = runFactory({text: "Kurswert", height: 24});
    runs = [
      first,
      after(first, SPLIT_GAP, {text: "1.005,00", height: 24})
    ];
    expect(cellsOfRow(rowOf(runs)).map((cell: PdfCellBox): string => cell.text)).toEqual(["Kurswert 1.005,00"]);
  });
