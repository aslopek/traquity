import {getDocument, PDFDocumentLoadingTask, PDFDocumentProxy, PDFPageProxy} from "pdfjs-dist";
 ...
 *
 * Parsing runs on pdf.js's configured worker, so a document that spins the parser occupies that worker and not the
 * window. The time budget is checked between pages, which bounds the document and no single page's parse.
 *
 * @param bytes the file's contents.
 * @param currency the one code an amount of this document may be denoted in; every other word stays text, so a
 *   figure stated in another currency is carried undenoted.
 * @param bounds the page count, run count and elapsed-time limits the parse stays within, each of them a failure
 *   reason once exceeded; defaults to {@link DEFAULT_PDF_EXTRACTION_BOUNDS}.
...
export async function extractPdf(
  ...
  try {
    ...
    for (let number: number = 1; number <= pdf.numPages; number++) {
      ...
      const page: PDFPageProxy = await pdf.getPage(number);
      ...
      pages.push({number, width, height, empty: runs.length === 0, rows: rowsOf(runs, currency)});
    }
...
function rowsOf(runs: PdfRun[], currency: string): PdfRow[] {
  return rowsOfRuns(runs).map((row: PdfRunRow): PdfRow => ({
    y: Number(row.y.toFixed(2)),
    cells: classifyCells(cellsOfRow(row), currency),
  }));
}
