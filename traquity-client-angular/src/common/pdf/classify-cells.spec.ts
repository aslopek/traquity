...
describe("classifyCells", (): void => {
  ...
  let currency: string;
  /** The boxes as they stood before the act, so no expectation is a view of the act's own output. */
  let boxes: PdfCellBox[];

  /** Classifies the current cells, snapshotting their boxes first. */
  function classify(): PdfCell[] {
    boxes = cells.map((cell: PdfCellBox): PdfCellBox => ({...cell}));
    return classifyCells(cells, currency);
  }
  ...
  it.each(["AM", "am", "aM", "Am", "A.M.", "a.m.", "A.m.", "a.M."] satisfies string[])
  ("takes in the morning meridiem printed behind a time as a word of its own, spelled %s", (meridiem: string): void => {
    cells = [cellFactory({text: `Handelszeit 10:30 ${meridiem}`})];
    expect(classify()).toEqual([{
      ...boxes[0],
      tokens: [{text: "10:30", label: "Handelszeit", time: "10:30:00", last: true}]
    }] satisfies PdfCell[]);
  });

  it.each(["PM", "pm", "pM", "Pm", "P.M.", "p.m.", "P.m.", "p.M."] satisfies string[])
  ("takes in the afternoon meridiem printed behind a time as a word of its own, spelled %s", (meridiem: string): void => {
    cells = [cellFactory({text: `Handelszeit 4:30 ${meridiem}`})];
    expect(classify()).toEqual([{
      ...boxes[0],
  ...
  it("takes in a meridiem printed a wide gap behind the time, in the next cell", (): void => {
    ...
    expect(classify()).toEqual([
      {...boxes[0], tokens: [{text: "4:30", label: "Handelszeit", time: "16:30:00", last: true}]},
      {...boxes[1], tokens: []}
    ] satisfies PdfCell[]);
  });
  ...
  describe("currency", (): void => {
    ...
    it("is absent where the code printed behind the amount is a different currency", (): void => {
    ...
    it("leaves a code that is another currency in the label it stands in", (): void => {
      cells = [cellFactory({text: "Dividende USD 2,87"})];
      expect(classify()).toEqual([{
        ...boxes[0],
        tokens: [{text: "2,87", label: "Dividende USD", magnitude: 2.87, digits: 2, last: true}]
      }] satisfies PdfCell[]);
    });

    it("is the code the request names, whichever currency that is", (): void => {
      currency = "USD";
      cells = [cellFactory({text: "Dividende 2,87 USD"})];
      expect(classify()).toEqual([{
        ...boxes[0],
    ...
    it("is not taken from the quoted half of an exchange-rate pair", (): void => {
      cells = [
        cellFactory({text: "Devisenkurs"}),
        cellFactory({text: "USD / EUR 1,1442", x: 300})
      ];
      expect(classify()).toEqual([
        {...boxes[0], tokens: []},
        {
          ...boxes[1],
          tokens: [{text: "1,1442", label: "USD / EUR", magnitude: 1.1442, digits: 4, last: true}]
        }
      ] satisfies PdfCell[]);
    });

    it("denotes the half of a converted payment printed in that code and no other half", (): void => {
      cells = [
        cellFactory({text: "Ausschüttung"}),
        cellFactory({text: "31,80", x: 200}),
        cellFactory({text: "USD", x: 300}),
        cellFactory({text: "27,45+ EUR", x: 400})
      ];
      expect(classify()).toEqual([
        {...boxes[0], tokens: []},
        {
          ...boxes[1],
          tokens: [{text: "31,80", label: null, magnitude: 31.8, digits: 2, last: true}]
        },
        {...boxes[2], tokens: []},
        {
          ...boxes[3],
          tokens: [{
            text: "27,45+", label: null, magnitude: 27.45, digits: 2, sign: "+", currency: "EUR", last: false
          }]
        }
      ] satisfies PdfCell[]);
    });
