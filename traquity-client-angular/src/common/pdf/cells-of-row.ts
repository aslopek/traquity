 ...
 * the cells on either side into one.
 */
 ...
 *
 * Both thresholds are multiples of the taller run's height, so the split follows the type being read and no
 * page-wide column grid: a row right-aligning a tax block at its own x still splits correctly. A filler run is
 * dropped and separates whatever stood on either side of it.
 */
export function cellsOfRow(row: PdfRunRow): PdfCellBox[] {
  ...
  return cells.map((cell: PdfCellBox): PdfCellBox => ({
    ...cell,
