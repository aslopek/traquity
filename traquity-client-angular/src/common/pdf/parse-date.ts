...
/**
 * Every reading of a printed date as `yyyy-MM-dd`, the likelier one first, across the notations the regexes above
 * state. An ISO timestamp's time is dropped, and a numeric date's two separators must be the same character, so
 * `14.03/2024` is no date and `1.005,00` cannot become one.
 *
 * Where day and month are both plausible, day first comes first, since that is what every notation but the American
 * one states. A slash or a hyphen carries the month-first reading as well, so `04/15/2024` states the 15th of April
 * once the calendar has refused the other order. **A dot never does**: no locale writes `MM.dd.yyyy`.
 *
 * A reading has to name a **day of the calendar**, February's length in the stated year included, so `31.02.2024`
 * is no date and `29.02.2024` is one. Eight digits in a row (`20240314`) are deliberately no date: they are an
 * order or account number far more often, and reading one as the other is a mistake nothing downstream can see.
 *
