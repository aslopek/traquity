...
const TIME: RegExp = /^(\d{1,2}):(\d{2})(?::(\d{2}))?(?:[:.,]\d+)?(?:\s*([AaPp])\.?[Mm]\.?)?$/;

/**
 * A printed time as `HH:mm:ss` on the 24-hour clock, zero-padded: `9:05` becomes `09:05:00` and `4:30 PM` becomes
 * `16:30:00`. A time printed without seconds reads on the full minute, and anything finer than a second is dropped,
 * whether a settlement prints it behind a fourth colon (`17:28:43:27`) or behind a decimal separator.
 *
 * Hours, minutes and seconds are range-checked, against the 12-hour clock where an `AM`/`PM` says the time was
 * printed on that one. What a page prints around the clock face — a bracket, the colon behind a label — is no part
 * of it.
 *
 * @returns time as `HH:mm:ss` or `null` for anything that is not that shape.
 */
