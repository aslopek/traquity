 ...
 * yield nothing: which of several a page is about is a question the shape alone cannot answer, so it answers
 * neither.
 *
 * The check digit is not verified, so a word of the right shape that is no ISIN counts as a candidate. The rule
 * above is what bounds that: a false candidate standing beside a real one yields nothing at all.
 *
