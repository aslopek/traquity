 ...
 * is the decimal point and every earlier one groups thousands, so `1.005,00` and `1,005.00` are both `1005`.
 *
 * A leading or trailing `-`/`+` marks the direction of a booking and is reported apart from the magnitude, since a
 * transaction's own direction is carried by its type. Brackets state that direction in the accounting notation US
 * brokers print, so `(9.90)` carries the sign `9,90-` does.
 *
 * The number of decimals is not bounded: a quantity is printed with as many as the broker executed it at
 * (`0,55814` is a share of an ETF), and reading two of them turns a fraction into a different number.
 *
 * One shape stays genuinely ambiguous: a lone separator with exactly three digits behind it, where `1.005` is a
 * thousand and five to one reader and a fraction to another. **Both readings are returned**, the grouped one
 * first. Behind a `0` no grouping is possible, so `0,132` is a fraction and nothing else.
 *
 * An ASCII space is deliberately no grouping separator: words are separated by spaces, so admitting one would join
 * two unrelated figures into a number the page never printed.
 *
