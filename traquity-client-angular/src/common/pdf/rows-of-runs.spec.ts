...
describe("rowsOfRuns", (): void => {
  ...
  it("puts two runs on one baseline into one row", (): void => {
    // the runs in the order they were given, so a reordering in place cannot change both sides alike
    const given: PdfRun[] = [...runs];

    expect(rowsOfRuns(runs)).toEqual([{y: 429, height: 10, runs: given}] satisfies PdfRunRow[]);
  });
