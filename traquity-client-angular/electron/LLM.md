...
electron/
  ...
  ai/
    ...
    transaction-extraction/
      transaction-extraction.js the document-extraction usecase: the grammar of token ids generated from the tokens the request
                                carries, the message the model reads, the schema its answer is parsed with, and the
                                one ISIN a page contains. Reads no notation of its own - see `architecture/ai.md`
                                ADR-013
...
(`prompts/<usecase>/`, a `default.md` plus a `<modelKey>.md` for each model that needs one of its own), the grammar its answer decodes under
(if needed), and the schema that answer is parsed with. A second usecase therefore leaves every module named in this section untouched.
`ai/transaction-extraction/` is the first one and further usecases get their own subdirectory.
...
- **Only one generation runs at a time, and the lock is per process, never per usecase.** The machine holds the weights of one model at a
  ...
  flag per usecase. Nothing queues: a generation holds a model and a context for seconds to minutes.
- **A model is loaded in the main process, not in a utility process.** `node-llama-cpp` tests a GPU-enabled binary in a child process before
  ...
  `app.on('ready')` puts a window on screen and runs `removePreviousLog()` over the log of the run in progress.See `architecture/ai.md`
  ADR-007 for the trade that buys, and nothing in the suite can catch a regression: it shows up as a flashing window on a packaged run.
...
(`architecture/ai.md` ADR-008) — plus **the literals that renderer read off it**, the currency the amounts are to be read in and the
catalogue key of the model to use. Behind it, `ai/transaction-extraction/transaction-extractor.js` resolves the prompt and the model,
generates the grammar from those literals and hands the request to `ai/local-inference.js`; nothing about it is written to
`traquity.config.json`.
...
(`maxTokens` being the one that explains truncated JSON). A request the *channel's* schema refuses is logged there too, written by
`ipc/ai-bridge.js`, since the extractor never sees such a request at all: a rejected `invoke` carries no outcome, so a bound the renderer
tripped over is named in the log or nowhere.

One of those bounds is worth knowing before another is added. A token's `label` is the prose a page prints in front of a figure, which is a
whole row of it — a settlement footer naming a board of directors runs past 150 characters — so it carries a bound of its own,
`MAXIMUM_AI_LABEL_LENGTH`, and not the tight one that holds a printed figure's `text` and `value`. Sizing it like a figure refuses entirely
ordinary documents, and the renderer has nothing to show for such a refusal but the sentence that points back at this log.

Two things about this usecase are not obvious and expensive to find:

**The ISIN is no part of the answer, and no part of this process** (`architecture/ai.md` ADR-013): the renderer reads it off the document
it parsed and resolves it against the securities it holds, so nothing here ever sees it. The grammar has no `isin`
rule, the answer schema no `isin` key and the packaged prompts no ISIN instruction, and a change that puts one back has to say what the
model would be selecting between. The rule this follows is the general one: a field needing no judgement about the page is read off the
document deterministically, and every field a page prints several candidates for stays the model's.

**A grammar literal and the prompt's reading rule are one decision made in two files, and one of them is in the other tier.** The
alternation a field decodes under is built from what the renderer's `src/common/pdf/` readers state (`architecture/ai.md` ADR-013), and the
prompt here tells the model how to read the very same words. Where the two disagree the model cannot state what it read, and a constrained
decode does not fail for it: the alternation admits the other numbers of the page, so an order number is emitted as the quantity and the
answer parses. That is why a page printing `Stück 0,55814` needs both the reader to keep five decimals and the prompt to stop saying two,
and why neither the notation nor the reading rule in `prompts/transaction-extraction/*.md` moves alone. A missing literal is never a
refusal - it is a wrong value. Nothing in this directory can fix one: a field the renderer did not read is a field no prompt reaches.
