 ...
 * dialog and never reads a member of it, which is why its type is a parameter.
 *
...
describe('aiDialogs', () => {
  ...
  beforeEach(() => {
    ...
    subjectUnderTest = createAiDialogs({dialog, getParentWindow: () => window});
  });

  it('returns the picked directory', async () => {
    await expect(subjectUnderTest.pickDownloadDirectory()).resolves.toBe(pickedDirectory);
  });

  it('opens a directory-only picker parented to the main window', async () => {
    await subjectUnderTest.pickDownloadDirectory();
  ...
  it('returns null when the dialog is cancelled', async () => {
    ...
    await expect(subjectUnderTest.pickDownloadDirectory()).resolves.toBeNull();
  });
