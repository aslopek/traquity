...
// one lock for every usecase, so a request in flight refuses any other, whatever it asks
const inferenceLock = createInferenceLock({
  runInference: localInference.run,
  log: logFile.write
});
...
async function resolveLlama() {
  ...
  return getLlama({
    build: 'never',
    // the library's own output would go to stdout, which a packaged app has nowhere to show
    logger: (level, message) => logFile.write(`[node-llama-cpp] ${level}: ${message}`)
  });
}

/**
 * Whether a failure was memory refusing an allocation. The class is the library's own, so a failure to reach it
 * answers `false`: what cannot be classified is not retried.
 *
 * @param {unknown} error
