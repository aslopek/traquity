...
/** @import {DocumentLiterals} from './ipc-schema.js' */
 ...
 * @property {(modelKey: string, document: string, literals: DocumentLiterals, currency: string) =>
...
function createAiBridge(options) {
  ...
  function register() {
    ...
    handle('ai:extractTransaction', async (_event, request) => {
      ...
      const {modelKey, document, literals, currency} = parsedRequest.data;
      return extractTransaction(modelKey, document, literals, currency);
    });
