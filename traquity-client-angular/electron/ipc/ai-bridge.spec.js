...
describe('aiBridge', () => {
  ...
  describe('ai:extractTransaction', () => {
    ...
    it('writes down which field it refused, a rejected call carrying no outcome to put it in', async () => {
      request = {...request, currency: 'Euro'};

      await expect(handleListenerFor('ai:extractTransaction')({}, request)).rejects.toThrow();

      expect(log.mock.calls).toEqual([[expect.stringContaining('currency')]]);
    });

    it('writes nothing down for a request it accepted', async () => {
      await handleListenerFor('ai:extractTransaction')({}, request);
