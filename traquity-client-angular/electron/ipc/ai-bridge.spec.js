...
describe('aiBridge', () => {
  ...
  const extractTransaction = jest.fn(
    /** @type {(modelKey: string, document: string, tokens: import('./ipc-schema.js').DocumentToken[], currency: string) =>
  ...
  const log = jest.fn(/** @type {(message: string) => void} */ (() => undefined));
  ...
  describe('ai:getState', () => {
    it('merges the registry\'s state with an empty verdict map for an empty catalogue', async () => {
      await expect(handleListenerFor('ai:getState')(undefined))
        .resolves.toEqual({isConfirmed: false, catalogue: [], models: {}, verdicts: {}, probeFailed: true});
    });
    ...
    it('reports probeFailed false once the capability probe resolves', async () => {
      ...
      await expect(handleListenerFor('ai:getState')(undefined))
        .resolves.toEqual({isConfirmed: false, catalogue: [], models: {}, verdicts: {}, probeFailed: false});
    });
  ...
  describe('ai:extractTransaction', () => {
    ...
    it('hands the request on', async () => {
      await handleListenerFor('ai:extractTransaction')({}, request);

      expect(extractTransaction)
        .toHaveBeenCalledWith(request.modelKey, request.document, request.tokens, request.currency);
    ...
    it('refuses a request stating more tokens than the bound admits', async () => {
      request = {...request, tokens: Array((2 ** 10) + 1).fill(request.tokens[0])};

      await expect(handleListenerFor('ai:extractTransaction')({}, request)).rejects.toThrow();
      expect(extractTransaction).not.toHaveBeenCalled();
    });

    it('refuses a token value longer than the bound admits', async () => {
      request = {...request, tokens: [{id: 1, kind: 'number', text: 'x', value: '2'.repeat(129), label: null}]};

      await expect(handleListenerFor('ai:extractTransaction')({}, request)).rejects.toThrow();
      expect(extractTransaction).not.toHaveBeenCalled();
    });

    it('refuses a request carrying no tokens key at all', async () => {
