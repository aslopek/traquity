 ...
 * the bytes actually written. What a caller does with a chunk that stays under the cap - hash it, count it, both
 * - is `onChunk`'s business alone; this module never looks inside a chunk.
 */
  ...
  return new Transform({
    transform(chunk, _encoding, callback) {
      ...
      if (totalBytes > maxBytes) {
        callback(new Error(`Stream exceeded ${maxBytes} bytes`));
        return;
      }
      onChunk(chunk);
      callback(null, chunk);
