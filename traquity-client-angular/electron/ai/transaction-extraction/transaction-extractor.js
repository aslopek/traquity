...
/** Prefixes every entry this module writes, so one run is one thread in a shared log. */
const LOG_PREFIX = '[transaction-extraction]';

/** What a caller is told for an answer that reached no transaction, whichever way it failed. */
...
function createTransactionExtractor(options) {
  ...
  /**
   * The answer is parsed and never trusted: the grammar constrains its shape, and the schema decides whether that
   * shape carries a transaction this app can use.
   *
   * @param {string} answer
