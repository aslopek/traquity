 ...
 * @param {number | undefined} settled the total the page states, absent where it states none
 * @returns {number | null} the line to drop, or `null` where no line equals the overshoot. Several that do carry
 *   the same value by construction, so which one is dropped cannot change the sum.
 */
