...
describe('createTransactionExtractor', () => {
   ...
   */
  let transaction;
  /** @type {import('../../ipc/ipc-schema.js').DocumentToken[]} */
  let tokens;
  ...
  beforeEach(() => {
    ...
    tokens = [
      {id: 1, kind: 'number', text: '1.700,00', value: '1700.00', label: 'Kurswert'}
    ];
    ...
    getState = jest.fn(() => ({
      ...
      catalogue: [],
      models: {
        [MODEL_KEY]: {path: MODEL_PATH, active: true},
        [OTHER_MODEL_KEY]: {path: OTHER_MODEL_PATH, active: false}
      }
    }));
  ...
  it('answers with the transaction the schema accepted', async () => {
    await expect(subjectUnderTest.extract(MODEL_KEY, DOCUMENT, tokens, CURRENCY))
  ...
  it('runs the model on the resolved prompt, the built message and the document\'s own grammar', async () => {
    await subjectUnderTest.extract(MODEL_KEY, DOCUMENT, tokens, CURRENCY);
  ...
  it('runs the model the key names, whichever one is active', async () => {
    ...
    expect(runInference).toHaveBeenCalledWith(expect.objectContaining({modelPath: MODEL_PATH}));
    expect(runInference).toHaveBeenCalledTimes(1);
  });
  ...
  it('builds the message out of the document and the currency it was given', async () => {
    ...
    expect(messageForMock).toHaveBeenCalledWith(DOCUMENT, tokens, CURRENCY);
    expect(messageForMock).toHaveBeenCalledTimes(1);
  });

  it('generates the grammar from the tokens the request carries', async () => {
    await subjectUnderTest.extract(MODEL_KEY, DOCUMENT, tokens, CURRENCY);

    expect(grammarForMock).toHaveBeenCalledWith(tokens);
  ...
  it('builds the transaction out of the lines the schema accepted', async () => {
    ...
    expect(transactionOfAnswerMock).toHaveBeenCalledWith(answeredLines, tokens);
  ...
  describe('refusals', () => {
    ...
    it('refuses an answer the sorting leaves without a date, a count or a gross', async () => {
      transactionOfAnswerMock.mockReturnValue(null);
