...
function tokenFactory(overrides = {}) {
  return {id: 1, kind: 'number', text: '1.700,00', value: '1700.00', label: 'Kurswert', ...overrides};
}
...
describe('grammarFor', () => {
  ...
  beforeEach(() => {
    tokens = [
      tokenFactory({id: 1, kind: 'date', value: '2024-02-02'}),
      tokenFactory({id: 2, kind: 'time', value: '14:05:00'}),
      tokenFactory({id: 3, value: '1700.00'}),
      tokenFactory({id: 4, value: '25.32'})
    ];
  });

  it('asks for every field of a transaction, each as the ids the page states', () => {
    expect(grammarFor(tokens).split('\n')[0]).toBe('root ::= "{" ws "\\"transactionType\\":" ws type ws'
  ...
  it('admits every date the page states as the one a transaction takes', () => {
    tokens = [...tokens, tokenFactory({id: 5, kind: 'date', value: '2024-02-05'})];

    expect(grammarFor(tokens)).toContain('date ::= "1" | "5"');
  });

  it('admits every amount as a quantity and as a line of any monetary field', () => {
    expect(grammarFor(tokens)).toContain('count ::= "3" | "4"');
    expect(grammarFor(tokens)).toContain('amount ::= "3" | "4"');
  });

  it('states a monetary field as a list, so a page stating several lines for one is stated as several', () => {
    expect(grammarFor(tokens)).toContain('ids ::= "[" ws ( amount ( ws "," ws amount )* ws )? "]"');
  });

  it('asks for no date where the page states none', () => {
    tokens = [tokenFactory({id: 1})];
  ...
  it('admits the empty list alone where the page states no amount', () => {
    tokens = [tokenFactory({id: 1, kind: 'date', value: '2024-02-02'})];

    expect(grammarFor(tokens)).toContain('ids ::= "[" ws "]"');
    expect(grammarFor(tokens)).not.toContain('count');
    expect(grammarFor(tokens)).not.toContain('amount');
  });

  it('states every rule on one line, since llama.cpp ends a rule at the first newline', () => {
    expect(grammarFor(tokens).split('\n')).toHaveLength(8);
  });
});
...
describe('messageFor', () => {
  ...
  it('states the currency, then the page, then the tokens to sort', () => {
    expect(messageFor('Kurswert  |  1.700,00 EUR', tokens, 'EUR')).toBe('Currency: `EUR`\n\nKurswert  |  1.700,00 EUR'
      + '\n\n--- values read off the page ---\n1. number 1700.00 — Kurswert (printed as 1.700,00)');
  });

  it('states the currency of an amount the page denotes', () => {
    tokens = [tokenFactory({currency: 'EUR'})];

    expect(messageFor('x', tokens, 'EUR')).toContain('1. number 1700.00 EUR — Kurswert (printed as 1.700,00)');
  });

  it('states a token the page names nothing', () => {
    tokens = [tokenFactory({label: null})];

    expect(messageFor('x', tokens, 'EUR')).toContain('1. number 1700.00 (printed as 1.700,00)');
  });
  ...
  it('states a date in the notation an answer uses, beside the text it was printed as', () => {
    ...
    expect(messageFor('x', tokens, 'EUR')).toContain('1. date 2025-01-02 — Valuta (printed as 02.01.2025)');
  });
...
describe('extractedAnswerSchema', () => {
  ...
  beforeEach(() => {
    answer = {transactionType: 'SELL', date: 1, count: 2, gross: [3], tax: [4, 5], fee: [], netProceedings: [], taxBase: []};
  });

  it('accepts a type and the ids every field takes', () => {
    expect(extractedAnswerSchema.safeParse(answer)).toEqual({success: true, data: answer});
  });

  it('accepts an answer taking no id at all, which a page stating none admits', () => {
    answer = {transactionType: 'SELL', gross: [], tax: [], fee: [], netProceedings: [], taxBase: []};

    expect(extractedAnswerSchema.safeParse(answer).success).toBe(true);
  });

  it('refuses an id that is no whole number', () => {
    answer = {...answer, gross: [1.5]};

    expect(extractedAnswerSchema.safeParse(answer).success).toBe(false);
  });

  it('refuses a transaction type outside the four', () => {
    answer = {...answer, transactionType: 'TRANSFER'};

    expect(extractedAnswerSchema.safeParse(answer).success).toBe(false);
  });

  it('refuses an id of zero, which numbers no token', () => {
    answer = {...answer, gross: [0]};

    expect(extractedAnswerSchema.safeParse(answer).success).toBe(false);
  });

  it('refuses a value stated beside the ids, which the model does not state', () => {
    answer = {...answer, grossValue: [1700]};
...
describe('transactionOfAnswer, a charge stated as itself and as the lines it was computed in', () => {
  ...
  beforeEach(() => {
    answer = {
      ...
      grossValue: [7.1],
      tax: [6.21, 0.34, 0.55], // the three withholdings the page computed the 7.10 in
      fee: [],
      netProceedings: [7.1],
      taxableBase: [36.26]
    };
  ...
  it('compares the lines against the charge in whole cents, as every other line of this module does', () => {
    answer = {
      ...answer,
      tax: [6.214, 0.34, 0.55] // 7.104, which is the charge to the cent the page states it in
    };
