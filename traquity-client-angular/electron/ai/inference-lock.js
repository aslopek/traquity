 ...
 *
 * A request arriving while one runs is refused and never queued: a generation holds a model and a context for
 * seconds to minutes, so a queue turns a refusal the caller can act on into an indefinite wait. A request runs to
 * an outcome; there is no cancellation.
 *
 * The lock is over the run and not over a kind of request: the machine holds the weights of one model at a time,
 * so what a refused request was going to ask does not matter.
 */

/** Prefixes every entry this module writes, so a refusal is one thread in a shared log. */
