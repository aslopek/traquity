...
code exactly as a settlement line does, so a value reading `number 5.73 EUR — Dividende pro Stück` is a price and no money that moved. The
...
`Geschäftstag`, `Auftragsdatum`, `Ausführung`, `Trade date`. For a dividend or a tax charge it is the day the money reached the account:
`Valuta`, `Wertstellung`, a sentence saying when the amount is credited or debited, `Zahlbarkeitstag`, `Zahltag`, `Zahlungstag`,
`Pay date`. Those words are examples and never the whole of it — a broker you have not seen before names its dates in words no list covers,
and the two rules below decide without needing any of them.
...
**A settlement date is struck out of a trade and of nothing else.** The paragraph above is about a `BUY` and a `SELL`: there the day the
money moved falls after the transaction, so it is the wrong day. On a `DIVIDEND` or a `TAX` charge the money moving **is** the transaction,
so `Valuta`, `Wertstellung` and the sentence booking the amount (`Den Betrag buchen wir mit Wertstellung 14.09.2026 zu Gunsten des Kontos`)
are exactly what you are looking for — they stay in, and being the last day the page names is what makes them win over a `Zahlbarkeitstag`
printed three days earlier.

**A trade's date and its time are one combination**, printed either at once (`Schlusstag/-Zeit 27.04.2026 17:14:36`) or in two halves that
...
wrong even where each looks right alone: a page printing `Schlusstag/-Zeit 27.04.2026 17:14:36` under a letter dated `28.04.2026` answers
`2026-04-27`, because the 27th is the date standing beside the time you named. Where a trade's page prints no time at all, there is nothing
...
`Ausschüttung | 13,23 | USD | 11,56+ EUR` reads as `13.23` with no code and `11.56 EUR` in the list, so the gross is `11,56` and never
`13,23`, however plainly the word `Ausschüttung` stands in front of the latter.
...
Den Betrag buchen wir mit Wertstellung 26.03.2026 zu Gunsten Ihres Kontos.
```
...
14. date 2026-03-26 — Den Betrag buchen wir mit Wertstellung (printed as 26.03.2026)
```
...
The page names two days and neither is struck out: id 1 announces the payment and id 14 books it. The 26th is the later of the two, so it is
