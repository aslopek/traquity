...
**One three-letter code can stand after a value, and it is the one the request names.** A value carrying it — `1005.00 EUR` — is a figure
the page denoted in that currency, printed beside those very digits. A value carrying nothing after it is every other case at once: a figure
the page denoted in **another** currency, and a rate, a quantity or a reference number denoted in none. Id 2 above is such a value, and the
page does print `USD` beside it.
...
- **A currency code, or nothing at all.** The page puts the amount in a column of its own, so the naming text opens the row: id 3 above is a
  ...
  page with you — and read the label there. A code standing *before* the dash is such a neighbouring column and says nothing about this
  value: what the value is denoted in is the code standing after it.
...
- **A value printed inside brackets is a marker and never an amount** — `(1)`, `(2)`, `(4)`, `(30%)`, `(731402)`: a footnote reference, a
...
prints rates, quantities and balances that are no amounts at all. **Neither of those carries a code**, so the two are one case to you: what
does not carry the requested code is not this transaction's money. Given `EUR` and
...
`gross` is `[4]` and `tax` is `[6]`. Ids 2 and 5 carry no code: the page states them in another currency, and naming either counts that
money twice. Id 3 carries none either and is a rate.

**A perfect label is not a code.** This is the trap a converted payment sets, and it is worth seeing once in full. The page prints one row:

```
Dividendengutschrift  |  137,14  |  USD  |  126,40+ EUR
```

and the values read off it are

```
18. number 137.14 — Dividendengutschrift (printed as 137,14)
19. number 126.40 EUR — USD (printed as 126,40+)
```

Given `EUR`, `gross` is `[19]`. The row prints its label first and its `USD` column in the middle, so both words land on the wrong figure:
`Dividendengutschrift` names the *foreign* amount, and `USD` reaches the converted one as a **label**. Id 18 is out however plainly
`Dividendengutschrift` stands in front of it, because no code stands after it; id 19 reads `EUR` there and is the gross, however foreign
the word in front of it looks.

So go through the list once before you name anything monetary and read what stands after each value. **Every id you name for `gross`, `tax`,
`fee`, `netProceedings` or `taxBase` has to carry the requested code** — and an id carrying none is another currency's figure as readily as
it is a rate, a quantity or a reference number, however well it is labelled. Where a page carries no amount with the requested code at all,
it states its currency somewhere else entirely; then the code decides nothing and the labels decide alone.

**A code makes nothing an amount, though.** A price per share, a limit and a per-share distribution are quoted in a currency and carry its
code exactly as a settlement line does, so a value reading `number 4.15 EUR — Dividende pro Stück` is a price and no money that moved. The
code narrows what may be named; *What belongs to no field* decides the rest.
...
to be one whose value is followed by the requested currency code in the list.** An id carrying none is another currency's figure, a rate, a
...
- **A withholding and the credit for that same withholding are one line printed twice.** A page states what a foreign state took and,
  ...
  ```
  Einbehaltene Quellensteuer 15 % auf 137,14 USD  |  18,96- EUR
  Anrechenbare Quellensteuer 15 % auf 126,40 EUR  |  18,96  EUR
  ```

  That is one withholding of 18,96 and not two. Name the line that was **taken** — the one printed with the sign — and never both. A line
...
3. **Otherwise the page only takes tax, and the answer is `TAX`.** A `Vorabpauschale`, a `Steuermitteilung`, a `Steuerliche Meldung`, a
   ...
   the account, with nothing bought, nothing sold and nothing paid out. The printed sign settles it — an `Ausmachender Betrag` of `10,27-`
   is money leaving — and so does the sentence booking it (`zu Lasten des Kontos`). A `Vorabpauschale` is no distribution: it is a tax on a
...
`Zahlbarkeitstag`, `Zahltag`, `Zahlungstag`, `Valuta`, `Wertstellung`, `Pay date`. Those words are examples and never the whole of it — a
broker you have not seen before names its dates in words no list covers, and the two rules below decide without needing any of them.
...
**A `DIVIDEND` or a `TAX` charge prints no time, so its dates decide by their order alone.** Strike out the dates that are about something
other than this payment: a financial year, the two ends of a printed period (`01.01.2026 - 31.12.2026`), a `Bestandsstichtag`, a record
date, an `Ex-Tag`, a rate's own date (`Devisenkursdatum`), a stock on a given day, a date beside an order or invoice number, and the date in
the letter's own header block beside the addressee and the account numbers. **Then take the latest of what is left** — the last day in the
calendar, never the first: a page announces a payment before it books it, so the last day it still names is the day the money moved. Where
several of the dates left over name the same day, they are one day and any of them answers it.

**A trade's date and its time are one combination**, printed either at once (`Schlusstag/-Zeit 09.03.2026 14:02:51`) or in two halves that
stand beside each other (`Auftragsdatum` with `Auftragszeit`, a `Handelsdatum` / `Handelsuhrzeit` pair of column headings). A settlement
prints two such combinations, because an order is **placed** and then **executed**, and the execution is the moment the trade happened. So
you sort them and count, and no word on the page decides it:

1. **List the combinations.** For every time the page prints, take the date belonging to it: the date inside the text beside that time where
   there is one, and otherwise the last date printed before it.
2. **Drop what is no moment of its own.** A time marked as a local time (`Ortszeit`, `Local time`) is an instant you already have, in
   another zone. Two entries with the same date and the same time are one combination, however far apart the page prints them.
3. **Sort what is left, oldest first** — by the date, and by the time within one date.
4. **Take the second oldest where two or more are left, and the only one where one is left.** The oldest is when the order was placed; the
   one after it is when it was executed.

**`date` is that combination's date and `time` is its time, both out of the one combination.** A date from one and a time from another is
wrong even where each looks right alone: a page printing `Schlusstag/-Zeit 09.03.2026 14:02:51` under a letter dated `10.03.2026` answers
`2026-03-09`, because the 9th is the date standing beside the time you named. Where a trade's page prints no time at all, there is nothing
to sort and the words above decide the date alone.

**`time`** — the time of the combination you settled on, and never a time out of another one. Most `DIVIDEND` and `TAX` pages print no time
at all; leave the field out there.
...
`Ausschüttung | 17,40 | USD | 15,05+ EUR` reads as `17.40` with no code and `15.05 EUR` in the list, so the gross is `15,05` and never
`17,40`, however plainly the word `Ausschüttung` stands in front of the latter.
...
- A rate or a price per share: `Kurs`, `Ausführungskurs`, `Zum Kurs von`, `Preis pro Stück`, `Dividende pro Stück`,
  `Vorabpauschale pro St.`, `Devisenkurs`, `Limit`, `Price per unit`, and every percentage. A price is quoted in a currency and carries its
  code, which makes it no amount.
...
```
--- values read off the page ---
1. date 2026-03-10 — Datum (printed as 10.03.2026)
2. number 100200300 — Depotnummer (printed as 100200300)
3. date 2026-03-09 — Schlusstag/-Zeit (printed as 09.03.2026)
4. time 14:02:51 — Schlusstag/-Zeit 09.03.2026 (printed as 14:02:51)
5. number 24 — Stück (printed as 24)
6. number 47.25 — Ausführungskurs (printed as 47,25)
7. number 1134.00 EUR — Kurswert (printed as 1.134,00)
8. number 8.75 EUR — Provision (printed as 8,75)
9. number 1142.75 EUR — Ausmachender Betrag (printed as 1.142,75)
```
...
Id 1 is the letter's own date and 3 is the trade's. Id 6 is the price one share cost and 7 the amount 24 of them came to. Id 9 is what was
charged once the fee was added: `1134.00 + 8.75`, so it is the settlement total and never the gross.
...
```
Dividendengutschrift  |  137,14  |  USD  |  126,40+ EUR
Umrechnung in EUR  |  126,40  |  EUR
Einbehaltene Quellensteuer 15 % auf 137,14 USD  |  18,96- EUR
Anrechenbare Quellensteuer 15 % auf 126,40 EUR  |  18,96  EUR
Kapitalertragsteuerpflichtige Dividende  |  126,40  |  EUR
Ausmachender Betrag  |  107,44+ EUR
Den Betrag buchen wir mit Wertstellung 13.02.2026 zu Gunsten Ihres Kontos.
```
...
```
--- values read off the page ---
1. date 2026-02-11 — Zahlbarkeitstag (printed as 11.02.2026)
2. number 32 — Stück (printed as 32)
3. number 137.14 — Dividendengutschrift (printed as 137,14)
4. number 126.40 EUR — USD (printed as 126,40+)
5. number 126.40 EUR — Umrechnung in EUR (printed as 126,40)
6. number 15 — Einbehaltene Quellensteuer (printed as 15)
7. number 137.14 — Einbehaltene Quellensteuer 15 % auf (printed as 137,14)
8. number 18.96 EUR — Einbehaltene Quellensteuer 15 % auf 137,14 USD (printed as 18,96-)
9. number 15 — Anrechenbare Quellensteuer (printed as 15)
10. number 126.40 EUR — Anrechenbare Quellensteuer 15 % auf (printed as 126,40)
11. number 18.96 EUR — Anrechenbare Quellensteuer 15 % auf 126,40 EUR (printed as 18,96)
12. number 126.40 EUR — Kapitalertragsteuerpflichtige Dividende (printed as 126,40)
13. number 107.44 EUR — Ausmachender Betrag (printed as 107,44+)
14. date 2026-02-13 — Den Betrag buchen wir mit Wertstellung (printed as 13.02.2026)
```
...
The page names two days and neither is struck out: id 1 announces the payment and id 14 books it. The 13th is the later of the two, so it is
the date — a `Zahlbarkeitstag` loses to the day the amount actually reached the account, and it loses on being the earlier day and on
nothing else.

Id 3 carries the label `Dividendengutschrift` and is **not** the gross: no code stands after its value, because the page states it in the
security's own currency. Id 4 is the same payment converted — `EUR` after its value, `USD` in front of it as a label — and id 5 is that
conversion stated a second time. Ids 6 and 9 are percentages, 7 and 10 the figures those percentages were applied to. Ids 8 and 11 are one
withholding printed twice — 8 is what was taken, 11 what may be credited — so only 8 is named. The arithmetic comes out:
`126.40 − 18.96 = 107.44`.
...
Stück 120  |  GLOBAL EQUITY UC.ETF ACC  |  IE00TRAQ1006
Steuerpflichtige Vorabpauschale  |  52,40+  |  EUR
Kapitalertragsteuer 24,45 % auf 36,68 EUR  |  8,97- EUR
Solidaritätszuschlag 5,5 % auf 8,97 EUR  |  0,49- EUR
Kirchensteuer 9 % auf 8,97 EUR  |  0,81- EUR
Ausmachender Betrag  |  10,27- EUR
```
...
`1` the `Zahlbarkeitstag`, `2` the `Stück 120`, `3` the `52,40`, `4`/`6`/`8` the three withholdings and `9` the `10,27`:
...
The charge **is** the transaction, so the `10,27` is the gross and the same id is the settlement total. The `52,40` is what the tax was
computed from and belongs in `taxBase`. The three withholdings add up to the `10,27` already named, so naming them as well would charge it
